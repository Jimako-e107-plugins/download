...
class plugin_download_admin extends e_admin_dispatcher
{
	...
	*/
	...
	protected $adminMenu = array(
		...
		'limits/prefs' 		=> array('caption' => DOWLAN_112, 'perm' => 'P'),
...
*/
	...
	function saveSettings()
	{
		...
		$expected_params = array(
			...
			'agree_text',
			'download_denied',
			'download_reportbroken',
			'download_security_mode',
			'download_security_expression',
			'download_security_link_expiry'
		);
...
class download_ui extends e_admin_ui
{
	...
	protected $pluginName		= 'download';
	//	protected $eventName		= 'members-'; // remove comment to enable event triggers in admin. 		
	protected $table			= '';
	protected $pid				= '';
}
...
class limits_ui extends download_ui
{
	...
	public function beforePrefsSave($new_data, $old_data)
	{
		...
		// Remove items with empty limit_classnum
		$tmp = array_filter($new_data['download_limit'], function ($item)
		{
			return !empty($item['limit_classnum']);
		});

		$new_data['download_limit'] = array();
		$new_data_unique = array();

		foreach ($tmp as $item)
		{

			// Check for division by zero
			if (!empty($item['limit_count_days']) && $item['limit_count_days'] != 0)
			{
				$item['limit_count_perday']  = $item['limit_count_num'] / $item['limit_count_days'];
			}
			else
			{
				// Handle the case where division is not possible
				$item['limit_count_perday'] = 0;
			}

			if (!empty($item['limit_bw_days']) && $item['limit_bw_days'] != 0)
			{
				$item['limit_bw_perday'] = $item['limit_bw_num'] / $item['limit_bw_days'];
			}
			else
			{
				// Handle the case where division is not possible
				$item['limit_bw_perday'] = 0;
			}


			// Use 'limit_classnum' as the key to overwrite previous entries
			$new_data_unique[$item['limit_classnum']] = $item;
		}
		$new_data['download_limit'] = array_values($new_data_unique);
...
/*
gen_id as limit_id, 
gen_datestamp as limit_classnum, 
gen_user_id as limit_bw_num, 
gen_ip as limit_bw_days, 
gen_intdata as limit_count_num, 
gen_chardata as limit_count_days
*/
class limits_form_ui extends download_form_ui
{
	public function download_limit($curVal, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'write': // Edit Page
				...
				if (!is_array($value))
				{
					$value = array(); // Wrap $value in an array if it isn't one
				}
				...
				$text .= "<tr>
					...
					<th class='fcaption'>" . DOWLAN_113 . "</th>
					<th class='fcaption'>" . DOWLAN_107 . "</th>
					<th class='fcaption'>" . DOWLAN_108 . "</th></tr>";

				$count = count($limitList);
				$new = count($limitList) + 1;
				$limitList[$new]['limit_id'] = count($limitList) + 1;
				$limitList[$new]['limit_classnum']	= 252;
				$limitList[$new]['limit_bw_num']	= "";
				$limitList[$new]['limit_bw_days']	= 1;
				$limitList[$new]['limit_count_num']	= "";
				$limitList[$new]['limit_count_days']	= 1;


				$id = 0;
				foreach ($limitList as $page => $val)
				{

					$id = $id + 1;
					$nameitem = 'limit_id';
					$actual_value = isset($value[$page]) ? $id : DOWLAN_114;
					$field = array('type' => 'hidden');
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $id, $field);
					$text .= "<tr><td>" . $actual_value . ": </td><td>";

					$nameitem = 'limit_classnum';
					$field = array('type' => 'userclass', 'writeParms' => array('classlist' => "no-excludes,guest,member,admin,classes,language,blank"));
					$actual_value = isset($value[$page][$nameitem]) ? $value[$page][$nameitem] : '';
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $actual_value, $field);
					$text .= "</td>";

					$text .= '<td>';
					$text .= '<div style="display: flex; align-items: center; gap: 5px;">';
					$nameitem = 'limit_count_num';
					$field = array('type' => 'number', 'data' => 'int',);
					$actual_value = isset($value[$page][$nameitem]) ? $value[$page][$nameitem] : '';
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $actual_value, $field);
					$text .= DOWLAN_109;
					$nameitem = 'limit_count_days';
					$field = array('type' => 'number', 'data' => 'int',);
					$actual_value = isset($value[$page][$nameitem]) ? $value[$page][$nameitem] : 1;
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $actual_value, $field);
					$text .= DOWLAN_110;
					$text .= "</div></td>";
					...
					$nameitem = 'limit_bw_num';
					$field = array('type' => 'number', 'data' => 'int',);
					$actual_value = isset($value[$page][$nameitem]) ? $value[$page][$nameitem] : '';
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $actual_value, $field);
					$text .= DOWLAN_109;

					$nameitem = 'limit_bw_days';
					$field = array('type' => 'number', 'data' => 'int',);
					$actual_value = isset($value[$page][$nameitem]) ? $value[$page][$nameitem] : 1;
					$text .= $this->renderElement('download_limit[' . $page . '][' . $nameitem . ']', $actual_value, $field);
					$text .= DOWLAN_110;
